import {
  ...
  /**
   * Handles the '/fim/completions' API request by selecting the appropriate provider(s) and making the request to them.
   *
   ...
   */
  export async function fimCompletionsHandler(c: Context): Promise<Response> {
    try {
      ...
      console.log('fimCompletion error', err.message);
      return new Response(
        JSON.stringify({
          status: 'failure',
