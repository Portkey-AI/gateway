...
function getProxyPath(
  ...
  if (proxyProvider === AI_21) {
    const endpoint = Providers[proxyProvider].api.getEndpoint(
      reqPath.replace("/", ""),
      requestBody.params.model
    );
    // Providers[proxyProvider].api.getEndpoint()
    return `${providerBasePath}${endpoint}`;
  }
...
export async function proxyHandler(
  ...
  if (requestHeaders["x-rubeus-config"]) {
    ...
    const camelCaseConfig: Config = convertKeysToCamelCase(config, [
      "override_params",
    ]);
  ...
  let retryCount = Math.min(
    parseInt(requestHeaders[HEADER_KEYS.RETRIES]),
    MAX_RETRIES
  );
