...
const addLogClient = (clientId:any, client:any) => {
  ...
  console.log(`New client ${clientId} connected. Total clients: ${logClients.size}`)
}

const removeLogClient = (clientId:any) => {
  logClients.delete(clientId)
  console.log(`Client ${clientId} disconnected. Total clients: ${logClients.size}`)
}
...
export const logger = () => {
  return async (c: Context, next: any) => {
    ...
    if (requestOptionsArray[0].requestParams.stream) {
      requestOptionsArray[0].response = {"message": "The response was a stream."}
    } else {
