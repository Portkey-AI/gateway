...
/**
 * Check if admin routes are explicitly enabled via environment variable
 */
const isAdminRoutesEnabled = (): boolean => {
  return Environment({}).MCP_ADMIN_ROUTES_ENABLED === 'true';
};

/**
 * Middleware to restrict routes to deployments with admin routes explicitly enabled
 * Returns 403 Forbidden if MCP_ADMIN_ROUTES_ENABLED is not set to 'true'
 */
const adminRoutesEnabledMiddleware = createMiddleware(async (c, next) => {
  if (!isAdminRoutesEnabled()) {
    logger.warn(
      `Admin route accessed but MCP_ADMIN_ROUTES_ENABLED is not enabled: ${c.req.path}`
    );
    return c.json(
      {
        error: 'forbidden',
        error_description:
          'Admin routes are not enabled. Set MCP_ADMIN_ROUTES_ENABLED=true to enable.',
      },
      403
    );
  }
...
// Apply admin routes enabled middleware to all admin routes
adminRoutes.use('*', adminRoutesEnabledMiddleware);
