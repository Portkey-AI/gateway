...
/**
 * Handle OAuth callback from external auth providers
 * This receives the authorization code from external auth providers and issues gateway tokens
 */
oauthRoutes.get('/external-callback', async (c) => {
  logger.debug('Received external auth callback', {
    url: c.req.url,
  });

  return await gw(c).handleExternalAuthCallback();
});
