...
describe('acuvity handler', () => {
  const cleanText = 'this is a test string for moderations';

  it('should fail if the apiKey is invalid', async () => {
    const context = createChatCompleteRequestContext(cleanText);
  ...
  it('should check fail if content is prompt_injection', async () => {
    ...
    const context = createChatCompleteRequestContext(promptInjectionText);
    const parameters = getParameters();
    const result = await acuvityHandler(
      context,
      parameters,
  ...
  it('should check fail if content has pii', async () => {
    ...
    const context = createChatCompleteRequestContext(piiText);
    const parameters = getParameters();
    const result = await acuvityHandler(
      context,
      parameters,
  ...
  it('should check pass if content only has pii redact', async () => {
    const piiRedactText =
      'Get a summary of stock market and send email to email address: abcd123@gmail.com';
    const context = createChatCompleteRequestContext(piiRedactText);
    const parameters = {
      credentials: testCreds,
  ...
  it('should check fail if content has pii redact and other detections', async () => {
    ...
    const context = createChatCompleteRequestContext(toxicPiiText);
    const parameters = {
      credentials: testCreds,
      ...getPIIRedactParameters(),
      ...getToxicityParameters(),
  ...
  it('should check pass if content only has  only redacted secrets', async () => {
    ...
    const context = createChatCompleteRequestContext(secretsText);
    const parameters = {
      credentials: testCreds,
  ...
  it('should check pass if content only has pii redact  on response', async () => {
    const piiResponseText =
      'Get a summary of stock market and send email to email address: abcd123@gmail.com';
    const context = createChatCompleteResponseContext(piiResponseText);
    const parameters = {
      credentials: testCreds,
      ...getPIIRedactParameters(),
    };
  ...
  it('should check fail if content has redact-pii and detect-secrets and  on response', async () => {
    ...
    const context = createChatCompleteResponseContext(complexResponseText);
    const parameters = {
      credentials: testCreds,
      ...getPIIRedactParameters(),
      ...getSecretsParameters(),
  ...
  it('should check fail if content has only secret detect', async () => {
    ...
    const context = createChatCompleteRequestContext(secretDetectText);
    const parameters = {
      credentials: testCreds,
      ...getSecretsParameters(),
    };
  ...
  it('should check fail if content has only pii detect', async () => {
    const piiDetectText =
      'Get a summary of stock market and send email to email address: abcd123@gmail.com and I hate you';
    const context = createChatCompleteRequestContext(piiDetectText);
    const parameters = {
      credentials: testCreds,
      ...getPIIParameters(),
    };
  ...
  it('should check fail if content has jb', async () => {
    ...
    const context = createChatCompleteRequestContext(jailbreakText);
    const parameters = {
      credentials: testCreds,
