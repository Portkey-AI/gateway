...
const helpers: PluginHandlerOptions = {} as PluginHandlerOptions;

export function setHelpers(pluginOptions: PluginHandlerOptions) {
  helpers.env = pluginOptions.env;
  helpers.getFromCacheByKey = pluginOptions.getFromCacheByKey;
  helpers.putInCacheWithValue = pluginOptions.putInCacheWithValue;
  helpers.externalServiceFetch = pluginOptions.externalServiceFetch;
  helpers.internalServiceFetch = pluginOptions.internalServiceFetch;
}

export const getAwsEndpointDomain = () =>
  helpers.env.AWS_ENDPOINT_DOMAIN || 'amazonaws.com';
...
function getAwsFilePath(fileName: string) {
  return helpers.env.HOME || helpers.env.USERPROFILE
    ? path.join(
        (helpers.env.HOME || helpers.env.USERPROFILE) as string,
        '.aws',
        fileName
      )
    : '';
...
export function getRegionFromEnv(): string {
  return helpers.env.AWS_REGION || helpers.env.AWS_DEFAULT_REGION;
}
...
function getCredentialsFromEnvironment(): AWSCredentials | undefined {
  if (helpers.env.AWS_ACCESS_KEY_ID && helpers.env.AWS_SECRET_ACCESS_KEY) {
    return {
      source: 'Environment Variables',
      accessKeyId: helpers.env.AWS_ACCESS_KEY_ID,
      secretAccessKey: helpers.env.AWS_SECRET_ACCESS_KEY,
      sessionToken: helpers.env.AWS_SESSION_TOKEN,
      awsRoleArn: helpers.env.AWS_ROLE_ARN,
      awsRegion: getRegionFromEnv() || 'us-east-1',
    };
  }
}

async function getCredentialsFromSharedCredentialsFile(): Promise<
...
> {
  ...
  try {
    ...
    const profile = helpers.env.AWS_PROFILE || 'default';
    ...
    if (credentials[profile]) {
      ...
      const awsCredentials: AWSCredentials = {
        ...
        secretAccessKey,
        sessionToken,
        awsRoleArn: awsRoleArn || helpers.env.AWS_ROLE_ARN,
        awsRegion: region || aws_region || getRegionFromEnv() || 'us-east-1',
      };
...
async function getCredentialsFromAwsConfigFile(): Promise<
  AWSCredentials | undefined
> {
  ...
  try {
    ...
    const profileName = helpers.env.AWS_PROFILE || 'default';
    const profile =
...
async function assumeRoleWithWebIdentity(
  ...
  const region = awsRegion || helpers.env.AWS_REGION || 'us-east-1';
  const response = await helpers.externalServiceFetch(
...
async function assumeRoleWithPodIdentity(
  ...
  const response = await helpers.externalServiceFetch(credentialFullUri, {
    method: 'GET',
    headers: {
      Authorization: token,
    },
...
async function getAssumedWebIdentityCredentials(awsRegion?: string) {
  if (helpers.env.AWS_WEB_IDENTITY_TOKEN_FILE && helpers.env.AWS_ROLE_ARN) {
    try {
      const effectiveRegion = awsRegion || getRegionFromEnv() || 'us-east-1';
      const roleArn = helpers.env.AWS_ROLE_ARN;
      const cacheKey = `assumed-web-identity-${helpers.env.AWS_WEB_IDENTITY_TOKEN_FILE}-role-${roleArn}-region-${effectiveRegion}`;
      const resp = await helpers.getFromCacheByKey(cacheKey);
      if (resp) {
        return resp;
      }
      const token = await fs.readFile(
        helpers.env.AWS_WEB_IDENTITY_TOKEN_FILE,
        'utf8'
      );
      ...
      if (credentials) {
        ...
        await helpers.putInCacheWithValue(cacheKey, merged, 300); //5 minutes
        return merged;
      }
    } catch (error) {
      console.error({
...
async function getPodIdentityCredentials(awsRegion?: string) {
  if (
    helpers.env.AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE &&
    helpers.env.AWS_CONTAINER_CREDENTIALS_FULL_URI
  ) {
    try {
      const effectiveRegion = awsRegion || getRegionFromEnv() || 'us-east-1';
      ...
      const cacheKey = `assumed-pod-identity-${helpers.env.AWS_CONTAINER_AUTHORIZATION_TOKEN_FILE}-region-${effectiveRegion}`;
      const resp = await helpers.getFromCacheByKey(cacheKey);
      if (resp) {
        return resp;
      }
      ...
      if (credentials) {
        ...
        await helpers.putInCacheWithValue(cacheKey, result, 300);
        return result;
      }
    } catch (error) {
...
async function getIRSACredentials(awsRegion?: string) {
  // get from web identity
  return getAssumedWebIdentityCredentials(awsRegion);
}

async function getCredentialsFromECSContainer() {
  if (helpers.env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI) {
    const cacheKey = `assumed-ecs-container-credentials`;
    const resp = await helpers.getFromCacheByKey(cacheKey);
    if (resp) {
      return resp;
    }
    const ecsUri = `http://169.254.170.2${helpers.env.AWS_CONTAINER_CREDENTIALS_RELATIVE_URI}`;
    const response = await helpers.externalServiceFetch(ecsUri, {
      method: 'GET',
    });
    if (!response.ok) {
    ...
    const derivedRegion =
      getRegionFromEnv() || (await getECSRegionFromMetadata()) || 'us-east-1';

    const derivedRoleArn =
      credentials.RoleArn ||
      helpers.env.AWS_ROLE_ARN ||
      (await getECSTaskRoleArnFromMetadata());

    await helpers.putInCacheWithValue(
      cacheKey,
      {
...
async function getIMDSRegion(token?: string): Promise<string> {
  const response = await helpers.externalServiceFetch(
...
async function getECSRegionFromMetadata(): Promise<string | null> {
  const uri =
    helpers.env.ECS_CONTAINER_METADATA_URI_V4 ||
    helpers.env.ECS_CONTAINER_METADATA_URI;
  if (!uri) return null;

  try {
    const resp = await helpers.externalServiceFetch(`${uri}/task`, {
      method: 'GET',
    });
...
async function getIMDSIdentityDocument(token?: string): Promise<any | null> {
  try {
    const resp = await helpers.externalServiceFetch(
      'http://169.254.169.254/latest/dynamic/instance-identity/document',
      {
        ...(token && {
          method: 'GET',
...
async function getECSTaskRoleArnFromMetadata(): Promise<string | null> {
  const uri =
    helpers.env.ECS_CONTAINER_METADATA_URI_V4 ||
    helpers.env.ECS_CONTAINER_METADATA_URI;
  if (!uri) return null;
...
async function getIMDSAssumedCredentials() {
  ...
  if (!region) {
    try {
      region = (await getIMDSRegion(imdsv2Token)) || 'us-east-1';
    } catch (e) {
  ...
  await helpers.putInCacheWithValue(cacheKey, credentials, 300); //5 minutes
  return credentials;
}
...
async function getSTSAssumedCredentials(
  ...
  const accessKeyId: string =
    accessKey || helpers.env.AWS_ASSUME_ROLE_ACCESS_KEY_ID || '';
  const secretAccessKey: string =
    secretKey || helpers.env.AWS_ASSUME_ROLE_SECRET_ACCESS_KEY || '';
  const region =
    awsRegion ||
    helpers.env.AWS_ASSUME_ROLE_REGION ||
    getRegionFromEnv() ||
    'us-east-1';
  const service = 'sts';
  ...
  let credentials: any;
  try {
    const response = await helpers.externalServiceFetch(url, {
      method: 'GET',
      headers: headers,
    });
...
export async function getAssumedRoleCredentials(
  ...
  if (!accessKeyId && !secretAccessKey) {
    try {
      ...
      console.error('Error while assuming source role', error);
      return null;
    }
  }
