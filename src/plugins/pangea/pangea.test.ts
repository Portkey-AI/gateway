...
describe('textGuardContentHandler', () => {
  it('should return an error if hook type is not supported', async () => {
    const context = createChatCompleteRequestContext('This is a message');
    const eventType = 'unsupported';
    const parameters = {};
    const result = await textGuardContentHandler(
      context,
      parameters,
      // @ts-expect-error - Testing with unsupported event type
      eventType,
      mockPluginHandlerOptions
    );
...
describe('pii handler', () => {
  const piiTestText = 'My email is abc@xyz.com and some random text';
