...
describe('phi handler', () => {
  ...
  const phiText = 'John Doe has a history of heart disease';
  ...
  it('should fail when text contains PHI', async () => {
    const context = createChatCompleteRequestContext(phiText);
    const parameters = { credentials: testCreds };
          ...
          messages: [
            {
              role: 'user',
              content: [
                { type: 'text', text: phiText },
                { type: 'text', text: `${phiText} and some random text` },
              ],
            },
          ],
        },
...
describe('pii handler', () => {
  const cleanText = 'this is a test string for moderations';
  const piiText = 'My email is abc@xyz.com and some random text';
          ...
          messages: [
            {
              role: 'user',
              content: [
                { type: 'text', text: 'My email is abc@xyz.com' },
  ...
  it('should detect and redact PII in response text', async () => {
    const context = createChatCompleteResponseContext(piiText);
    const parameters = {
      redact: true,
      credentials: testCreds,
    };
...
describe('toxicity handler', () => {
  it('should fail if beforeRequestHook is used', async () => {
    ...
    const result = await toxicityHandler(
      context,
      parameters,
      eventType,
      mockPluginHandlerOptions
