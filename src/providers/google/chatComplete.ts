...
export const GoogleChatCompleteResponseTransform: (response: GoogleGenerateContentResponse | GoogleErrorResponse, responseStatus: number) => ChatCompletionResponse | ErrorResponse = (response, responseStatus) => {
        ...
        return {
            ...
            choices: response.candidates?.map((generation, index) => {
                ...
                if (generation.content.parts[0]?.text) {
                    message = { role: "assistant", content: generation.content.parts[0]?.text }
                        ...
                        tool_calls: [
                            {
                                ...
                                function: {
                                    ...
                                    arguments: JSON.stringify(generation.content.parts[0]?.functionCall.args)
                                }
                            }
                        ]
                  }
    ...
    return {
        error: {
            message: `Invalid response recieved from google: ${JSON.stringify(response)}`,
            type: null,
            param: null,
            code: null
        },
...
export const GoogleChatCompleteStreamChunkTransform: (response: string) => string = (responseChunk) => {
    // console.log("stream", responseChunk)
